...
def rpn_graph(feature_map, anchors_per_location, anchor_stride):
    ...
    rpn_bbox = keras.layers.Lambda(lambda t: tf.reshape(t, [tf.shape(input=t)[0], -1, 4]))(x)
...
def rpn_class_loss_graph(rpn_match, rpn_class_logits):
    ...
    indices = tf.compat.v1.where(keras.backend.not_equal(rpn_match, 0))
    ...
    loss = keras.backend.sparse_categorical_crossentropy(target=anchor_class,
                                             ...
                                             from_logits=True)
    loss = keras.backend.switch(tf.size(input=loss) > 0, keras.backend.mean(loss), tf.constant(0.0))
    return loss
...
def rpn_bbox_loss_graph(config, target_bbox, rpn_match, rpn_bbox):
    ...
    indices = tf.compat.v1.where(keras.backend.equal(rpn_match, 1))
...
def mrcnn_bbox_loss_graph(target_bbox, target_class_ids, pred_bbox):
    ...
    # Smooth-L1 Loss
    loss = keras.backend.switch(tf.size(input=target_bbox) > 0,
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    loss = keras.backend.switch(tf.size(input=y_true) > 0,
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            gt_boxes = keras.layers.Lambda(lambda x: norm_boxes_graph(
        ...
        if mode == "training":
            ...
            if not config.USE_RPN_ROIS:
                ...
                # Normalize coordinates
                target_rois = keras.layers.Lambda(lambda x: norm_boxes_graph(
            ...
            # Losses
            rpn_class_loss = keras.layers.Lambda(lambda x: rpn_class_loss_graph(*x), name="rpn_class_loss")(
                [input_rpn_match, rpn_class_logits])
            rpn_bbox_loss = keras.layers.Lambda(lambda x: rpn_bbox_loss_graph(config, *x), name="rpn_bbox_loss")(
                [input_rpn_bbox, input_rpn_match, rpn_bbox])
            class_loss = keras.layers.Lambda(lambda x: mrcnn_class_loss_graph(*x), name="mrcnn_class_loss")(
                [target_class_ids, mrcnn_class_logits, active_class_ids])
            bbox_loss = keras.layers.Lambda(lambda x: mrcnn_bbox_loss_graph(*x), name="mrcnn_bbox_loss")(
                [target_bbox, target_class_ids, mrcnn_bbox])
            mask_loss = keras.layers.Lambda(lambda x: mrcnn_mask_loss_graph(*x), name="mrcnn_mask_loss")(
